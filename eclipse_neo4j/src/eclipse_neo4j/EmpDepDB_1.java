...
public class EmpDepDB_1 {
	...
	public void setUp() throws IOException {
		...
		graphDb = new GraphDatabaseFactory().newEmbeddedDatabase(SOCIETY_DB);
	...
	public void createNodespace() {
		try (Transaction tx = graphDb.beginTx()) {
			...
			Relationship rel = thomas.createRelationshipTo(sophie, RelTypes.WORKS_WITH);
	...
	private Node getSocieteNode() {
		return graphDb.getNodeById(socNodeId).getSingleRelationship(RelTypes.SOC_NODE, Direction.OUTGOING).getEndNode();
	}
	...
	private Traverser getColleagues(final Node person) {
		TraversalDescription td = graphDb.traversalDescription().breadthFirst()
				.relationships(RelTypes.WORKS_WITH, Direction.OUTGOING).evaluator(Evaluators.excludeStartPosition());
